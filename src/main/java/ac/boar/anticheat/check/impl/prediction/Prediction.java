...
public class Prediction extends OffsetHandlerCheck {
    ...
    public void onPredictionComplete(float offset) {
        ...
        if (player.unvalidatedTickEnd.distanceTo(player.velocity) < player.getMaxOffset() && !headBonk && !recentHeadBonk) {
        ...
        if (!MathUtil.sameDirectionHorizontal(actual, predicted) && !headBonk && !recentHeadBonk) {
        ...
        if (speedDiff > 1.0E-3F && !headBonk && !recentHeadBonk) {
